...
class SensorPublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(LaserScan, '/range_5', 10)
    ...
    def dynamic_joint_states_callback(self, msg):
        for interface_value in msg.interface_values:
            if 'Sensor5' in interface_value.interface_names:
                sensor_value = interface_value.values[4]  # Assuming only one value for simplicity
                scan_msg = LaserScan()
                scan_msg.header.stamp = self.get_clock().now().to_msg()
                scan_msg.header.frame_id = 'tof_5'  # Set appropriate frame ID
